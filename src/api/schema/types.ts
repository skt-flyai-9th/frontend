...
export interface Me {
  ...
  createdAt?: string;
  /**
   * 🔴 **이미 등록한 가게가 있으면 그 id** (2026-08-31 백엔드 추가, PR #133).
   *
   * 없으면 `null` 입니다. 로그아웃하면 기기에 저장된 `storeId` 가 지워지므로,
   * 다시 로그인했을 때 **가게가 있는지 알 방법이 이 값밖에 없습니다.**
   * 없으면 가게가 이미 있는 사장님도 매장 등록부터 다시 하게 됩니다.
   *
   * ⚠️ "내 가게 목록" API 는 없습니다 — 앱이 **가게 하나** 전제로 만들어져 있고
   *    서버도 그렇습니다. 여러 개가 생기면 그때 목록 API 가 필요합니다.
   */
  storeId?: number | null;
}
